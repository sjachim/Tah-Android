...
public class PwmControlFragment extends Fragment implements SeekBar.OnSeekBarChangeListener, CompoundButton.OnCheckedChangeListener {
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        ...
        switch (seekBar.getId()) {
            case R.id.seekBar13:
                updateConnectionState(",13,", mImgViewLed, mTxtSeekValue, String.valueOf(progress));
                break;
            case R.id.seekBar11:
                updateConnectionState(",11,", mImgViewLed11, mTxtSeekValue11, String.valueOf(progress));
                break;
            case R.id.seekBar10:
                updateConnectionState(",10,", mImgViewLed10, mTxtSeekValue10, String.valueOf(progress));
                break;
            case R.id.seekBar9:

                updateConnectionState(",9,", mImgViewLed9, mTxtSeekValue9, String.valueOf(progress));

                break;
            case R.id.seekBar6:
                updateConnectionState(",6,", mImgViewLed6, mTxtSeekValue6, String.valueOf(progress));
                break;
            case R.id.seekBar5:
                updateConnectionState(",5,", mImgViewLed5, mTxtSeekValue5, String.valueOf(progress));
                break;
            case R.id.seekBar3:
                updateConnectionState(",3,", mImgViewLed3, mTxtSeekValue3, String.valueOf(progress));
                break;
        }
    }
    ...
    private void updateConnectionState(final String pin, final ImageView img, final TextView txtview, final String progressval) {
        getActivity().runOnUiThread(new Runnable() {
            @Override
            public void run() {
                if (sw.isChecked()) {
                    ((Selector) getActivity()).writeData("2"+pin+String.valueOf(progressval)+"R", false);
                    img.setAlpha((float) (Float.parseFloat(String.valueOf(progressval)) / 255.0));
                    txtview.setText(String.valueOf(progressval));
                } else {
                    ((Selector) getActivity()).writeData("1"+pin+String.valueOf(progressval)+"R", false);
                    img.setAlpha((float) (Float.parseFloat(String.valueOf(progressval)) / 255.0));
                    txtview.setText(String.valueOf(progressval));
                }
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        switch (buttonView.getId()) {
            case R.id.swanologservo:
                if (isChecked) {
                    mSeekBar.setMax(179);
                    mSeekBar11.setMax(179);
                    mSeekBar10.setMax(179);
                    mSeekBar9.setMax(179);
                    mSeekBar6.setMax(179);
                    mSeekBar5.setMax(179);
                    mSeekBar3.setMax(179);
